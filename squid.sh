...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 20.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/aresvu/installsquid/main/squid.conf
    if [ -f /sbin/iptables ]; then
        /sbin/iptables -I INPUT -p tcp --dport 31289 -j ACCEPT
        /sbin/iptables-save
    fi
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/aresvu/installsquid/main/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 31289 -j ACCEPT
    /sbin/iptables-save
